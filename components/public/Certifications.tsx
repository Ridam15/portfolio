...
const getAchievementIcon = (category?: string): React.ComponentType<{ className?: string }> => {
      ...
      >
        ...
        <div className="absolute top-4 right-4">
          <motion.div
            className={cn(
              'p-3 rounded-full',
              isExpired ? 'bg-red-500/10 border border-red-500/30' : 'bg-cyan-500/10 border border-cyan-500/30'
            )}
            animate={isHovered ? { scale: 1.1, rotate: 5 } : { scale: 1, rotate: 0 }}
            transition={{ duration: 0.3 }}
          >
            {certification.icon ? (
              <span className="text-2xl" role="img" aria-label="certification badge">
                {certification.icon}
              </span>
            ) : (
              <Award className={cn('w-6 h-6', isExpired ? 'text-red-400' : 'text-cyan-400')} />
            )}
          </motion.div>
        </div>
        ...
        <GlassCardContent>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-2">
            ...
            <span>Issued: {formatDate(certification.issueDate, { year: 'numeric', month: 'short' })}</span>
          </div>
          ...
          {certification.expirationDate && (
            <div className={cn(
              ...
              <span>
                ...
                {formatDate(certification.expirationDate, { year: 'numeric', month: 'short' })}
              </span>
            </div>
          )}
        ...
        <GlassCardContent>
          ...
          {achievement.date && (
            <div className="flex items-center gap-2 text-sm text-gray-400 mb-3">
              <Calendar className="w-4 h-4" />
              <span>{formatDate(achievement.date, { year: 'numeric', month: 'long' })}</span>
            </div>
          )}
...
const CertificationsAchievementsSkeleton: React.FC = () => {
  return (
    <section id="certifications" className="relative py-16 md:py-24 lg:py-32 container mx-auto px-4">
      {/* Heading Skeleton */}
...
const Certifications: React.FC<CertificationsProps> = ({ 
  ...
  const certifications: Certification[] = mockCertifications || data?.certifications || [];
  const achievements: Achievement[] = mockAchievements || data?.achievements || [];
  ...
  const activeCertifications = certifications.filter(
    cert => !cert.expirationDate || new Date(cert.expirationDate) > new Date()
  ).length;
  ...
  if (hasError) {
    return (
      <section id="certifications" className={cn('relative py-16 md:py-24 lg:py-32 container mx-auto px-4', className)}>
        <motion.h2
    ...
    return (
      <section id="certifications" className={cn('relative py-16 md:py-24 lg:py-32 container mx-auto px-4', className)}>
        ...
        <div className="text-center">
          ...
          <p className="text-gray-400 text-lg mb-2">No certifications or achievements available yet.</p>
          <p className="text-gray-500 text-sm">Add your credentials to Firestore to see them here.</p>
        </div>
      </section>
    );
  }
    ...
    >
          ...
          <GlassCard variant="bordered" className="px-6 py-4" enableHover>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-400">Active Certifications</div>
              </div>
            </div>
          </GlassCard>
        )}
      ...
      {achievements.length > 0 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={isInView ? { opacity: 1 } : { opacity: 0 }}
          transition={{ duration: 0.6, delay: certifications.length > 0 ? 0.6 : 0.4 }}
        >
          <div className="flex items-center gap-3 mb-6">
